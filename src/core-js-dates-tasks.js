...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (Date.parse(dateEnd) - Date.parse(dateStart)) / 86400000 + 1;
}
...
function formatDate(date) {
  ...
  const hoursTime = hours > 12 ? hours % 12 : hours;
  const minutes =
    dateNew.getMinutes() > 9
      ? dateNew.getMinutes()
      : `0${dateNew.getMinutes()}`;
  const seconds =
    dateNew.getSeconds() > 9
      ? dateNew.getSeconds()
      : `0${dateNew.getSeconds()}`;
  return `${dateNew.getMonth() + 1}/${Number(date.slice(8, 10))}/${dateNew.getFullYear()}, ${hoursTime}:${minutes}:${seconds} ${time}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstDayOfYear = startYearDate.getDay();
  const startYearDateInMillisec = Date.parse(startYearDate);
  const lastDayInMillisec = Date.parse(lastDay);
  const days = (lastDayInMillisec - startYearDateInMillisec) / 86400000;
  let result = 0;

  for (let i = days; i > 0; i -= 7) {
    result += 1;
    if (i - 7 <= 0) {
      if (firstDayOfYear + i > 6) {
        result += 1;
      }
    }
  }
...
function getNextFridayThe13th(date) {
  ...
  for (let i = startMonth; i < 24; i += 1) {
    const year = i < 12 ? startYear : startYear + 1;
    const month = i % 12;
    const nextThirtFriday = new Date(year, month, 13);

    if (result === undefined) {
      if (nextThirtFriday.getDay() === 5) {
        result = nextThirtFriday;
      }
    }
  }
  return result;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (let i = periodStart; i <= periodEnd; ) {
    ...
    for (let j = 0; j < countWorkDays; j += 1) {
      const arrPeriodEnd = periodEnd.split('-');
      const arrI = i.split('-');
      if (arrI[1] <= arrPeriodEnd[1]) {
        result.push(i);
      }

      workDay.setDate(workDay.getDate() + 1);
      day = workDay.getDate();
      month = workDay.getMonth() + 1;
      i = `${day > 9 ? day : `0${day}`}-${month > 9 ? month : `0${month}`}-${workDay.getFullYear()}`;
    }
...
function isLeapYear(date) {
  ...
  return newDate % 400 === 0 || (newDate % 4 === 0 && newDate % 100 !== 0);
}
